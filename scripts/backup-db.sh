...
if $DRY_RUN; then
  log "DRY RUN: Would run pg_dump against $POSTGRES_TARGET_DISPLAY"
  log "DRY RUN: Would save to ${BACKUP_FILE}"
...
log "Backup SHA-256: ${BACKUP_SHA256}"
